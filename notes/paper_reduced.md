...
- default SFT path may be local-machine-specific (`/Users/avalanche/...`) and must be changed for this machine.

## 12. Useful File Map

Read in this order:
1. `notes/paper_reduced.md`: this file.
2. `notes/eda.md`: current dataset facts.
3. `notes/routine.md`: project diary and caveats.
4. `src/minionerec_goodreads/scripts/rqvae/data_preprocess.py`: data construction.
5. `src/minionerec_goodreads/scripts/rqvae/text2emb.py`: item embedding.
6. `src/minionerec_goodreads/models/rqvae.py`: RQ-VAE.
7. `src/minionerec_goodreads/scripts/rqvae/generate_sid.py`: SID export.
8. `src/minionerec_goodreads/dataset/sft_dataset.py`: SFT prompt tasks.
9. `src/minionerec_goodreads/models/sft.py`: SID token addition and train modes.

Important outputs:
- `data/processed/rqvae/goodreads.item.json`
- `data/processed/rqvae/goodreads.item2id.json`
- `data/processed/rqvae/goodreads.emb-bge.npy`
- `data/processed/rqvae/goodreads.index.json`
- `data/processed/rqvae/train.csv`
- `data/processed/rqvae/valid.csv`
- `data/processed/rqvae/test.csv`

## 13. Next Best Engineering Steps

If continuing reproduction, do these before adding new research ideas:
1. Fix environment/lock issue so `uv run` and Makefile work consistently.
2. Reconcile processed CSV header names with `sft_dataset.py` or regenerate data from current `data_preprocess.py`.
3. Verify or fix RQ-VAE `in_dim` vs embedding dim 2560.
4. Train/generate SIDs and report codebook usage, collision count, reconstruction loss.
5. Change SFT pretrained model path to a valid local Qwen checkpoint.
6. Run a tiny SFT smoke test with `max_train_samples` and new-token-only.
7. Implement constrained SID generation + HR/NDCG evaluation.
8. Only then consider QLoRA/full SFT and GRPO.
